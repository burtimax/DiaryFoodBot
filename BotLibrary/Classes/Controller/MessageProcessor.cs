    ...
    public class MessageProcessor
    {
        ...
        public void ProcessCurrentMessage(Object inboxMessage)
        {
            ...
            try
            {
                ...
                bool approveUser = this.Methods.CheckUserBeforeMessageProcess(mes.ChatId);
                ...
                if (resultPreProcess.NeedProcessMessage)
                {
                    ...
                    hop = method?.Invoke(instance, new object[] {mes.Bot, mes}) as Hop;
                }
        ...
        public void ProcessCallback(TelegramBotClient bot, CallbackQuery callback, long chatId)
        {

            try
            {
                //Проверим пользователя если вдруг его нет в базе, то добавить его.
                bool approveUser = this.Methods.CheckUserBeforeMessageProcess(chatId);
                //Если пользователь не прошел проверку, то не обрабатываем сообщение. Вызываем метод из PullMethods
                if (approveUser == false)
                {
                    return;
                ...
                Hop hop = method?.Invoke(instance, new object[] {bot, chatId, callback, callback.Data}) as Hop;
